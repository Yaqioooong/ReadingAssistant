...
- 2026-09-08：真实使用复盘修复——①精确标识符兜底（P-002/P002 字面等价，语义全灭时按规范化 token 捞 chunk，vector/hybrid 双通道）②BM25 规范化 token ③问答缓存标识符变体复用（近似复问直接回旧答案+引用）④会话元问题路由（“我上一个问题”直接查历史，不进 RAG）。
- 2026-09-08：PDF 表格支持：pdfplumber 布局感知 + 表格检测（无表格页走布局文本；有表格页剔除表格区域文字流、表格转 Markdown 原子块），pypdf 兜底极小/非常规 PDF。
- 2026-09-07：表格结构化落地：docx 表格按 body 顺序提取转 Markdown（%%TABLE_BEGIN/END%% 哨兵），chunk 层作为不可切分单元整表入库（block_type=table）；pdf 表格暂未支持（pypdf 无结构）。
